...
type Tool = { id: number; name: string; cat: string; tags: string[]; desc: string; url: string; logo: string; color: string; tier: string };
...
const TOOLS: Tool[] = [
  ...
  { id:13, name:"Vercel v0",      cat:"UI Generation",        tags:["code","ui","frontend","react","design","no-code","components","website","landing page"],                          desc:"Generate full UI components from text prompts",           url:"v0.dev",             logo:"▲", color:"#ffffff",  tier:"core"      },
  { id:14, name:"Notion AI",      cat:"Productivity",         tags:["notes","documents","project","team","wiki","organize","planning","business","docs"],                              desc:"AI built into your workspace for docs & planning",        url:"notion.so",          logo:"📋", color:"#ffffff",  tier:"essential" },
...
export default function Stax() {
  ...
  const [stack, setStack] = useState<Tool[]>([]);
  ...
  const sendMessage = async (userText: string) => {
    ...
    try {
     const res = await fetch("/api/chat", { 
      ...
      let parsed: { title?: string; summary?: string; tool_ids?: number[]; message?: string };
      try {
        parsed = JSON.parse(raw.replace(/```json|```/g, "").trim());
      } catch {
        parsed = { title: "Your Stack", summary: "Built for your goals.", tool_ids: [1, 7, 9, 14], message: "Here's a solid starting stack tailored for you!" };
      }
      const recs = (parsed.tool_ids || []).map((id: number) => TOOLS.find(t => t.id === id)).filter((t): t is Tool => t !== undefined);
      setStack(recs);
      ...
      {phase === "landing" && (
        <div style={{ position: "relative", zIndex: 10, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 24px 0", textAlign: "center" }}>
          ...
          <p className="fadein-2" style={{ fontSize: 18, color: "rgba(255,255,255,.45)", maxWidth: 460, lineHeight: 1.7, marginBottom: 48 }}>
            Over 2,000 AI tools exist. Most people waste weeks figuring out which ones they actually need. Stax finds your perfect toolkit in 10 seconds.
          </p>
          <div className="fadein-3" style={{ marginBottom: 20 }}>
          ...
          <div className="fadein-4" style={{ display: "flex", gap: 8, marginBottom: 72, flexWrap: "wrap", justifyContent: "center" }}>
            {["🚀 Replaces hours of research", "🎯 Curated by AI, not ads", "📋 Copy & share your stack", "⚡ New tools added weekly"].map((t, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 100, padding: "8px 16px", fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,.5)" }}>{t}</div>
            ))}
          </div>
          ...
          <div style={{ width: "100%", maxWidth: 520, background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 24, padding: "32px", marginBottom: 80 }}>
            ...
            <p style={{ fontSize: 14, color: "rgba(255,255,255,.4)", marginBottom: 24, lineHeight: 1.6 }}>Save your stacks, get weekly AI tool updates, and access the full tool database.</p>
            {!emailSent ? (
              <form onSubmit={handleEmail} style={{ display: "flex", gap: 10 }}>
                ...
                <button type="submit" style={{ background: "linear-gradient(135deg,#FF6B35,#FFD23F)", color: "#080808", border: "none", borderRadius: 12, padding: "12px 24px", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>Join Waitlist</button>
              </form>
            ) : (
              <div style={{ textAlign: "center", padding: "16px", background: "rgba(6,255,165,.1)", border: "1px solid rgba(6,255,165,.3)", borderRadius: 12, color: "#06FFA5", fontWeight: 700, fontSize: 15 }}>✓ You&apos;re on the list!</div>
            )}
          </div>
        </div>
      )}
          ...
          <div style={{ width: "100%", maxWidth: 640 }}>
            ...
            {messages.length === 0 && (
              <div className="fadein-1" style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 28, justifyContent: "center" }}>
                {SUGGESTIONS.map((s, i) => (
                  <button key={i} className="sugg-pill" onClick={() => sendMessage(s)} style={{ background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 100, padding: "10px 18px", fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,.6)", cursor: "pointer", fontFamily: "inherit", transition: "all .15s" }}>{s}</button>
                ))}
              </div>
            )}
            {messages.length > 0 && (
              <div style={{ maxHeight: 320, overflowY: "auto", display: "flex", flexDirection: "column", gap: 14, marginBottom: 20, padding: "4px" }}>
                {messages.map((m, i) => (
                  <div key={i} style={{ maxWidth: "85%", padding: "13px 18px", borderRadius: m.role === "user" ? "20px 20px 6px 20px" : "20px 20px 20px 6px", fontSize: 15, lineHeight: 1.6, fontWeight: 500, alignSelf: m.role === "user" ? "flex-end" : "flex-start", background: m.role === "user" ? "linear-gradient(135deg,#FF6B35,#FFD23F)" : "rgba(255,255,255,.07)", color: m.role === "user" ? "#080808" : "#fff", border: m.role === "user" ? "none" : "1px solid rgba(255,255,255,.08)" }}>{m.content}</div>
                ))}
                {loading && (
            ...
            <div className="fadein-2" style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
              ...
              <button onClick={() => sendMessage(input)} disabled={!input.trim() || loading} style={{ width: 52, height: 52, borderRadius: 14, border: "none", background: input.trim() && !loading ? "linear-gradient(135deg,#FF6B35,#FFD23F)" : "rgba(255,255,255,.08)", color: input.trim() && !loading ? "#080808" : "rgba(255,255,255,.3)", fontSize: 22, cursor: input.trim() && !loading ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", transition: "all .2s", flexShrink: 0, fontFamily: "inherit" }}>→</button>
            </div>
        ...
        <div style={{ position: "relative", zIndex: 10, padding: "100px 24px 80px", display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ width: "100%", maxWidth: 820 }}>
            <div className="fadein" style={{ textAlign: "center", marginBottom: 16 }}>
              ...
              <h2 style={{ fontSize: "clamp(2.2rem,6vw,4rem)", fontWeight: 900, letterSpacing: "-.05em", marginBottom: 12, background: "linear-gradient(135deg,#FF6B35,#FFD23F,#06FFA5)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{stackTitle}</h2>
                ...
                return (
                  <a key={tool.id} href={`https://${tool.url}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}>
                    <div className="tool-card" style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 20, padding: 22, position: "relative", overflow: "hidden", transition: "all .2s ease", cursor: "pointer" }}>
                      ...
                      <div style={{ position: "absolute", top: 16, right: 16, width: 28, height: 28, borderRadius: "50%", background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "rgba(255,255,255,.3)" }}>{i + 1}</div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: ".08em", padding: "4px 12px", borderRadius: 100, background: tier.bg, color: tier.text, border: `1px solid ${tier.border}` }}>{tier.label}</span>
            ...
            <div className="fadein-3" style={{ background: "linear-gradient(135deg,#111 0%,#0d0d0d 100%)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 24, padding: 32, marginBottom: 32, position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ position: "relative", zIndex: 2 }}>
                ...
                <div style={{ fontSize: 12, color: "rgba(255,255,255,.25)", fontWeight: 600 }}>getstax.ai · Build yours free</div>
              </div>
            </div>
            <div className="fadein-4" style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", marginBottom: 48 }}>
              <button onClick={copyStack} style={{ background: copied ? "rgba(6,255,165,.15)" : "linear-gradient(135deg,#FF6B35,#FFD23F)", color: copied ? "#06FFA5" : "#080808", border: copied ? "1px solid rgba(6,255,165,.4)" : "none", borderRadius: 14, padding: "14px 32px", fontSize: 15, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", minWidth: 220, transition: "all .2s" }}>
                {copied ? "✓ Copied to clipboard!" : "📋 Copy & Share My Stax"}
              </button>
              <button className="ghost-btn" onClick={() => setPhase("chat")} style={{ background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.12)", color: "rgba(255,255,255,.6)", borderRadius: 14, padding: "14px 24px", fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", transition: "all .2s" }}>Refine My Stack ↺</button>
              <button className="ghost-btn" onClick={reset} style={{ background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.12)", color: "rgba(255,255,255,.6)", borderRadius: 14, padding: "14px 24px", fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", transition: "all .2s" }}>Start Over</button>
            </div>
            <div className="fadein-5" style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 24, padding: 32, textAlign: "center" }}>
              ...
              <p style={{ color: "rgba(255,255,255,.4)", fontSize: 14, marginBottom: 24, lineHeight: 1.6 }}>Join Stax Pro — save your stacks, get weekly new tool alerts, and compare costs.</p>
              {!emailSent ? (
                <form onSubmit={handleEmail} style={{ display: "flex", gap: 10, maxWidth: 420, margin: "0 auto" }}>
                  <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="your@email.com" required style={{ flex: 1, background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.12)", borderRadius: 12, padding: "12px 16px", color: "#fff", fontSize: 14, fontFamily: "inherit", transition: "border-color .2s" }} />
                  <button type="submit" style={{ background: "linear-gradient(135deg,#FF6B35,#FFD23F)", color: "#080808", border: "none", borderRadius: 12, padding: "12px 24px", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>Join Free</button>
                </form>
              ) : (
                <div style={{ padding: "14px", background: "rgba(6,255,165,.1)", border: "1px solid rgba(6,255,165,.3)", borderRadius: 12, color: "#06FFA5", fontWeight: 700, fontSize: 15, maxWidth: 420, margin: "0 auto" }}>✓ You&apos;re on the list!</div>
              )}
            </div>
          </div>
        </div>
